...
def register_tools(server: FastMCP, backend) -> None:
    ...
    async def ffmpeg_prores_encoder_avoptions(
        ...
        # Run CLI subcommand: 
        cmd_args: list[str] = []
        if mbs_per_slice is not None:
            if mbs_per_slice:
                cmd_args.append("-mbs_per_slice")
        if profile is not None:
            if profile:
                cmd_args.append("-profile")
        if vendor is not None:
        ...
        if bits_per_mb is not None:
            if bits_per_mb:
                cmd_args.append("-bits_per_mb")
        if quant_mat is not None:
            if quant_mat:
                cmd_args.append("-quant_mat")
        if alpha_bits is not None:
            if alpha_bits:
                cmd_args.append("-alpha_bits")
        return await backend.run_subcommand("", cmd_args)
